...

{
    ...
    var connection = builder.Environment.IsDevelopment()
        ? configuration["ConnectionStrings:AZURE_SQL_Local"]
        : Environment.GetEnvironmentVariable("AZURE_SQL_CONNECTIONSTRING");
    builder.Services.AddDbContext<PlantDbContext>(
        options =>
    ...
    var blobConnection = builder.Environment.IsDevelopment()
        ? configuration["Azure:BlobConnString"]
        : Environment.GetEnvironmentVariable("Azure_Blob_ConnectionString");
...

{
    ...
    app.UseCors(policy =>
    {
        policy
            .WithOrigins(
                "http://localhost:5174",
                "https://ashy-stone-00b16bb03.4.azurestaticapps.net"
            )
            .AllowAnyMethod()
            .AllowAnyHeader()
            .AllowCredentials();
